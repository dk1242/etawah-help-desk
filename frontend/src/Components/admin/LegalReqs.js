...
import { DeleteLegalReqs, getLegalReqs } from "./ApiAdmin";
...
const LegalReqs = () => {
  ...
  const DeleteRequest = (reqId) => () => {
    console.log(reqId);
    DeleteLegalReqs(user._id, token, { id: reqId })
      .then((data) => {
        if (data.err) {
  ...
  const loadOxygenReqs = async () => {
    await getLegalReqs(user._id, token)
      .then((data) => {
        console.log(data);
        if (data.error) {
          setError(data.error);
            ...
            reqs.length > 0 ? (
              reqs.map((req) => (
                <Col lg={4} style={{ border: "1px solid" }}>
                  ...
                  <br />
                  <h4>
                    <span style={{ textDecoration: "underline" }}>Problem</span>
                    : {req.problem}
                  </h4>
                  <br />
                  <h4>
